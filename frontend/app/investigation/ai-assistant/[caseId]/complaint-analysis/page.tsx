...
import { LucideIcon, Mic, FileText, Image as ImageIcon, Loader2, CheckCircle2, AlertCircle, Circle } from "lucide-react";
...
// ---------- status badge (right side of card header) ----------
...
function StatusBadge({
  ...
  if (loading) {
    return (
      <span className="flex items-center gap-1.5 rounded-full bg-maroon-50 px-3 py-1 text-xs font-medium text-maroon-700">
  ...
  return (
    <div className="rounded-lg border border-gold-200 bg-white p-5">
      <div className="flex items-start justify-between gap-3">
        ...
        <StatusBadge loading={loading} error={error} hasResult={!!result} />
      </div>

      <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
        <div className="flex flex-wrap items-center gap-3">
          <label
            className="cursor-pointer rounded-md border-2 border-gold-400 bg-white px-4 py-2 text-sm
                       font-medium text-ink-700 hover:bg-gold-50 transition-colors"
          >
            Choose File
            <input
              type="file"
